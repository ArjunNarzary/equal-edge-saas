...
CREATE TABLE IF NOT EXISTS "country_group_discounts" (
	...
	"discount_peracantage" real NOT NULL,
...
CREATE TABLE IF NOT EXISTS "country_groups" (
	...
	"name" text NOT NULL,
	"recommended_descount_percentage" real,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
...
CREATE TABLE IF NOT EXISTS "product_customizations" (
	...
	"class_prefix" text,
	"product_id" uuid NOT NULL,
	"location_message" text DEFAULT 'Hey! It looks like you are from <b>{countery}</b>. We support Parity Purchasing Power, so if you need it, use code <b>“{coupon}”</b> to get <b>{discount}%</b> off.' NOT NULL,
